const searchPhone = () => {
    ...
    if (searchText == '') {
        ...
        const url = `https://api.unsplash.com/search/photos?client_id=${access_key}&query=${searchText}&per_page=1000`;
        // const url = `https://api.unsplash.com/search/photos?query=${searchText}`;
        // console.log(url);
        fetch(url)
            .then(res => res.json())
            .then(data => {
...
const displayResult = phones => {
    ...
    phones.forEach(phone => {
        ...
        div.innerHTML = ` <div class="card border-0 shadow">
                            <img src="${phone.urls.thumb}" class="w-50 mx-auto my-4" alt="...">
                            <div class="card-body text-center">
                                <h6 class="card-title">${phone.brand}</h6>
                                <h5 class="card-title">User Name: ${phone.user.name}</h5>
